...
function FCSWriteStream(fcs, readableStream) {
   ...
   this._w = {
      state: 'header',
      eventsNeeded: ('asNone' === fcs.meta.dataFormat) ? 0 : fcs.meta.eventsToRead || FCS.DEFAULT_VALUES.eventsToRead,
      ...
      prepareForData: function(buffer) {
         ...
         return this.eventsNeeded * (this.readParameters.bigSkip + this.readParameters.bytesPerEvent) +
...
FCSWriteStream.prototype._write = function(chunk, inEncodingIgnored, callback) {
   ...
   while (('done' !== _w.state) && (this.bytesRead >= _w.bytesNeeded)) {
      ...
      switch (_w.state) {
         ...
         case 'text'  :
            var string = buffer.toString(_w.encoding, fcs.header.beginText, fcs.header.endText);
            ...
            _w.isAnalysisSegmentBeforeData = _w.isAnalysisThere && (fcs.header.beginAnalysis < fcs.header.beginData)
         ...
         case 'analysis':
            var string = buffer.toString(_w.encoding, fcs.header.beginAnalysis, fcs.header.endAnalysis);
   ...
   if (('done' !== _w.state))
     ...
     if (this.readableStream) {
        ...
        this.emit('finish');
         ...
         callback();  // can't turn off the spigot, keep reading
   }
